...
def create_imports_json(starting_directory, custom_modules_directory, json_file_path):
    ...
    # Filter out dictionary tables that do not start with the Python file names
    valid_files = [os.path.splitext(file)[0] for file in os.listdir(custom_modules_directory) if file.endswith('.py')]
    imports_dict = {key: value for key, value in imports_dict.items() if key in valid_files}
...
custom_modules_directory = r'F:\Stuff\PythonFilesIntoExes\C_GuideToOrganizeOutput'
